 ...
 *
 * This is the canonical "vaft" (Video Ad-block, Foiled by Twitch) script from
 * pixeltris/TwitchAdSolutions (MIT), the uBlock-Origin variant, used verbatim
 * except for: (1) the leading uBO resource header line is removed, and (2) a
 * single window.postMessage in updateAdblockBanner that feeds the extension's
 * per-tab "ads blocked" counter via the existing __SHIELDS_AD_BLOCKED__ relay.
 *
 * Source: https://github.com/pixeltris/TwitchAdSolutions (vaft/vaft-ublock-origin.js)
 * Mechanism: hooks the player's HLS Web Worker and swaps to an ad-free backup
 * stream (via alternate playerType) during stitched ad breaks, with player-reload
 * and buffering recovery. Constants here track Twitch's private API; when Twitch
 * changes it, re-sync from upstream.
 */
(function() {
    ...
    // Shields integration: feed the per-tab "ads blocked" counter on the rising
    // edge of an ad break, relayed by cosmetic-observer.ts -> service worker.
    let shieldsAdActive = false;
    const twitchWorkers = [];
    ...
    function updateAdblockBanner(data) {
        // Shields integration: count one ad-block event per ad break (rising edge).
        if (data.hasAds && !shieldsAdActive) {
            shieldsAdActive = true;
            try { window.postMessage({ type: '__SHIELDS_AD_BLOCKED__', count: 1 }, '*'); } catch (e) {}
        } else if (!data.hasAds) {
            shieldsAdActive = false;
        }
